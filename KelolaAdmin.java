...
public class KelolaAdmin {
    ...
    public static void tambahMenu(Menu[] menu) {
        ...
        while (true) {
            ...
            System.out.println("Masukkan kategori menu baru:");
            ...
            System.out.println("Berhasil menambahkan menu: " + namaMenu);
            Pelanggan.tampilDaftarMenu(menu);

            System.out.println("Apakah yakin untuk menambahkan menu baru?");
            // user konfirmasi ya / tidak
            boolean selesaidiUbah = masukkanKonfirmasi();

            if (selesaidiUbah){
                break;
            }
        }
        Main.manajemenMenu(menu);
    }
    ...
    public static void ubahHarga(Menu[] menu) {
        ...
        while (true) {
            ...
            System.out.println( );
            System.out.print("Masukkan nomor menu (Ubah Harga): ");
            int nomorMenu = masukkanNomorMenu(menu);
            System.out.print("Masukkan harga menu (Ubah Harga):");
            int hargaMenu = masukkanHargaMenu();

            System.out.println("Apakah anda yakin ingin mengubah harga menu?");
            // user konfirmasi ya / tidak
            boolean ya = masukkanKonfirmasi();

            if (ya){
                UbahMenu(menu, nomorMenu, hargaMenu);
                System.out.println("Menu berhasil diubah: " + menu[nomorMenu-1].getNama() + " = " + menu[nomorMenu-1].getHarga());
            } else {
                System.out.println("Menu gagal diubah");
            }

            tampilDaftarMenuDenganIndexNomor(menu);

            System.out.println("Apakah yakin untuk mengubah harga menu?");
            // user konfirmasi ya / tidak
    ...
    private static void UbahMenu(Menu[] menu, int nomorMenu, int hargaMenu) {
        for (Menu m : menu) {
    ...
    public static void hapusMenu(Menu[] menu) {
        ...
        while (true) {
            ...
            System.out.println("Apakah anda yakin ingin menghapus menu?");
            // user konfirmasi ya / tidak
            boolean ya = masukkanKonfirmasi();

            if (ya){
                System.out.println("Menu berhasil dihapus: " + menu[nomorMenu-1].getNama());
                menu = MenuDihapus(menu, nomorMenu);
            } else {
                System.out.println("Menu gagal dihapus");
            }


            System.out.println("Apakah yakin untuk menghapus menu?");
            // user konfirmasi ya / tidak
            boolean selesaiUbah = masukkanKonfirmasi();
    ...
    private static Menu[] MenuDihapus(Menu[] menu, int nomorMenu) {
        Menu[] menuBaru = new Menu[menu.length - 1];
